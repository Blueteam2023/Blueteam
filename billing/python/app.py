...
def prov():
	if request.method == "GET":
		...
		if PROV_NAME == f'[["{GIVEN_PROV_NAME}"]]':
			cursor.execute("SELECT id FROM Provider where name=(%s)",
			               (GIVEN_PROV_NAME,))
...
def provid(id):
	...
	if request.method == "GET":
		...
		if PROV_NAME == f'[["{GIVEN_PROV_NAME}"]]':
			...
			cursor.execute("UPDATE Provider set name=(%s) where id=(%s)",
			               (GIVEN_PROV_NAME, id,))
			connection.close()
    ...
    if request.method == "POST":
        ...
        if option == 'upload':
            ...
            if not all(col in df.columns for col in required_columns):
                 raise ValueError(
                     f"Error: the DataFrame must have the columns {required_columns}!")
            ...
            for row in df.itertuples():
                cursor.execute(
                    f"INSERT INTO Rates (product_id, rate, scope) VALUES ('{row.product_id}', '{row.rate}', '{row.scope}')")
            connection.commit()
            cursor.close()
            connection.close()
...
def truck_info():
	if request.method == "POST":	
		try:
			...
			from_param = request.form['from']
			#if to_param is None:
			to_param = request.form['to']

			if from_param == "1st of month at 000000":
				now = datetime.now()
				from_param = datetime(now.year, now.month, 1)
				from_param = from_param.strftime("%Y%m%d%H%M%S")
			else:
				from_param = datetime.strptime(from_param, "%Y%m%d%H%M%S")

			if to_param == "now":
				to_param = datetime.now()
				to_param = to_param.strftime("%Y%m%d%H%M%S")
			else:
		...
		if result:
			...
			# database=MYSQL_DB_NAME
			# )
			request1 = "SELECT truckTara FROM transactions WHERE truck = %s ORDER BY ABS(TIMEDIFF(datetime, %s)) LIMIT 1"
			cursor.execute(request1, (id, date_time))
			tara = cursor.fetchone()
			request2 = "SELECT id FROM transactions WHERE datetime BETWEEN %s AND %s AND truck = %s"
			cursor.execute(request2, (from_param, to_param, id))
			sessions = set(cursor.fetchall())
			cursor.close()
			connection.close()
			return jsonify({"id": id, "tara": tara, "sessions": sessions})
		else:
			abort(404)
    
    # renvoyer la page HTML
	return render_template('datruckid.html')
